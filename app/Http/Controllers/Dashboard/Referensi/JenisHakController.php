...
class JenisHakController extends Controller
{
    ...
    {
        // $this->authorizeResource(JenisHak::class, 'user');
    }
    ...
    {
        $models = JenisHak::filter($request->get('filters'))
        ...
        return view('dashboard.referensi.jenis_hak.index', compact('models'));
    }
    ...
    {
        $model = empty($id) ? new JenisHak : $this->findModel(['id' => $id]);

        return view('dashboard.referensi.jenis_hak.form', compact('model'));
    }
    ...
    {
        $model = JenisHak::where($params)->firstOrFail();
        return $model;
    }
}
